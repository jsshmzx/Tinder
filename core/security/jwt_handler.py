...
def create_temp_token(subject: str, purpose: str, expires_minutes: int | None = None) -> str:
    """创建临时 JWT token，仅用于特定目的（如注册完成）。

    与 create_access_token 的区别：
    - payload 含 purpose 声明，get_temp_user 依赖据此放行
    - 默认使用 TEMP_TOKEN_EXPIRE_MINUTES 配置
    ...
    if expires_minutes is None:
        expires_minutes = settings.TEMP_TOKEN_EXPIRE_MINUTES
